...
def clamp(target):
    return -1 if target < 0 else 9 if target > 8 else target


def comp1(player, x, y):
    return x < abs(y) if player == WHITE else x > (8 - abs(y))
...
class Simplified_Backgammon:
    ...
    def restore_state(self, old_state):
        self.board, self.off, self.players_positions = old_state.board[:], old_state.off[:], old_state.players_positions[:]
        self.state = BackgammonState(board=self.board, off=self.off, players_positions=self.players_positions)
    ...
    def move_to_go_home(self, player, r, out_of_home):
        ...
        if len(out_of_home) == 1:  # there is 1 position where there is/are checker(s) out of home
            ...
            if sum(self.board[out][0]) <= 3:
                ...
                if self.is_valid(player, t1):
                    if t1 in home_positions:  # I used 1 die to move 1 checker from the only position out of home board, to home board
                        if sum(self.board[out][0]) == 1:
                            move = (move1,)
                        elif sum(self.board[out][0]) == 2:
                            move = (move1, move1)
                        elif sum(self.board[out][0]) == 3:
                            move = (move1, move1, move1)
                    else:
                        if self.is_valid(player, t2) and sum(self.board[out][0]) == 1:
            ...
            if (sum(self.board[out1][0]) + sum(self.board[out2][0])) <= 3:
                ...
                if self.is_valid(player, t11) and self.is_valid(player, t12):
                    ...
                    if t11 in home_positions and t12 in home_positions:
                        ...
                        if sum(self.board[out1][0]) == 1 and sum(self.board[out2][0]) == 2:
                            move = (move11, move12, move12)
                        elif sum(self.board[out2][0]) == 1 and sum(self.board[out1][0]) == 2:
                            move = (move11, move11, move12)

                    elif t11 in home_positions and sum(self.board[out1][0]) == 1 and sum(self.board[out2][0]) == 1:
                        if self.is_valid(player, t22):
                            if t22 in home_positions:
                                move = (move11, move12, move22)

                    elif t12 in home_positions and sum(self.board[out1][0]) == 1 and sum(self.board[out2][0]) == 1:
            ...
            if self.is_valid(player, out1 + r) and self.is_valid(player, out2 + r) and self.is_valid(player, out3 + r) \
                    and sum(self.board[out1][0]) == 1 and sum(self.board[out2][0]) == 1 and sum(self.board[out3][0]) == 1 \
    ...
    def could_bear_off(self, player, roll):
        ...
        tot = sum([sum(self.board[position][0]) for position in self.players_home_positions[player] if player == self.board[position][1]])
        threshold = 6 - 1 if len(roll) == 2 else 6 - 3
        return tot >= (threshold - sum(self.off[player]))
    ...
    def is_valid(self, player, target):
        if 0 <= target < NUM_POINTS:
            return sum(self.board[target][0]) < 2 or self.board[target][1] == player
        return False
    ...
    def add_checker(self, action, player, knocked_off=False):
        if knocked_off:
            ...
            # 5 -> 6
                ...
                print(f"ERROR: Too many checkers at {action}, terminating")
                exit(1)
        elif action == -1 or action == 9:
            if self.off[player] == [0, 0, 0, 0, 0, 0]:
                self.off[player] = [1, 0, 0, 0, 0, 0]
            elif self.off[player] == [1, 0, 0, 0, 0, 0]:
                self.off[player] = [1, 1, 0, 0, 0, 0]
            elif self.off[player] == [1, 1, 0, 0, 0, 0]:
                self.off[player] = [1, 1, 1, 0, 0, 0]
            elif self.off[player] == [1, 1, 1, 0, 0, 0]:
                self.off[player] = [1, 1, 1, 1, 0, 0]
            elif self.off[player] == [1, 1, 1, 1, 0, 0]:
                self.off[player] = [1, 1, 1, 1, 1, 0]
            elif self.off[player] == [1, 1, 1, 1, 1, 0]:
                self.off[player] = [1, 1, 1, 1, 1, 1]
        else:
            # 0 -> 1
            if self.board[action][0] == [0, 0, 0, 0, 0, 0]:
                self.board[action] = ([1, 0, 0, 0, 0, 0], player)
            # 1 -> 2
            elif self.board[action][0] == [1, 0, 0, 0, 0, 0]:
                self.board[action] = ([1, 1, 0, 0, 0, 0], player)
            # 2 -> 3
            elif self.board[action][0] == [1, 1, 0, 0, 0, 0]:
                self.board[action] = ([1, 1, 1, 0, 0, 0], player)
            # 3 -> 4
            elif self.board[action][0] == [1, 1, 1, 0, 0, 0]:
                self.board[action] = ([1, 1, 1, 1, 0, 0], player)
            # 4 -> 5
            elif self.board[action][0] == [1, 1, 1, 1, 0, 0]:
                self.board[action] = ([1, 1, 1, 1, 1, 0], player)
            # 5 -> 6
            elif self.board[action][0] == [1, 1, 1, 1, 1, 0]:
                self.board[action] = ([1, 1, 1, 1, 1, 1], player)
            else:
                print(f"ERROR: Too many checkers at {action}, terminating")
                exit(1)
    ...
    def get_double_moves(self, player, roll, single_moves):
        ...
        if len(single_moves) > 0:
            ...
            moves.update([((s, t), (s, t)) for (s, t) in single_moves if sum(self.board[s][0]) >= 2])
        return moves
    ...
    def can_bear_off(self, player):
        ...
        return sum(tot) == (6 - sum(self.off[player]))
    ...
    def get_triple_moves(self, player, roll, double_moves):
        ...
        if len(double_moves) > 0:
            for (m1, m2) in double_moves:
                ...
                if self.is_valid(player, t1 + roll) and ((t1 != s2) or (sum(self.board[t1][0]) > 0 and self.board[t1][1] == player)):
                ...
                for s in self.players_positions[player]:
                    ...
                    if self.is_valid(player, t):
                        if (sum(self.board[s][0]) > 2 and ((s, t) == m1 or (s, t) == m2)) or ((s, t) != m1 and (s, t) != m2):
                            moves.add((m1, m2, (s, t)))

                        if (m1 != m2) and sum(self.board[s][0]) > 1 and ((s, t) == m1 or (s, t) == m2):
                            moves.add((m1, m2, (s, t)))
    ...
    def get_bear_off_plays(self, player, roll):
        ...
        if self.can_bear_off(player):
            ...
            if s1 in active_positions:
                move = (s1, s1 + r1)
                plays.add((move,))
                src = [s for s in active_positions if s1 != s or not (s1 == s and sum(self.board[s1][0]) == 1)]
                plays.update([(move, single) for single in self.get_single_moves(player, roll=r2, player_src=src)])

            if s2 in active_positions:
                move = (s2, clamp(s2 + r2))
                plays.add((move,))
                src = [s for s in active_positions if s2 != s or not (s2 == s and sum(self.board[s2][0]) == 1)]
            ...
            if len(highest_src) == 0:
                ...
                if s1_best is not None:
                    ...
                    for s in active_positions:
                        if self.is_valid(player, s + r2) and not (s == s1_best and sum(self.board[s1_best][0]) == 1):
                            plays.add((move, (s, s + r2)))

                    if sum(self.board[s1_best][0]) == 1:
                    ...
                    if self.is_valid(player, s1_best + r2) and sum(self.board[s1_best][0]) == 1:
                ...
                if self.is_valid(player, s + r2) and sum(self.board[s][0]) == 1:
                ...
                if self.is_valid(player, s + r1) and sum(self.board[s][0]) == 1:
        ...
        if self.can_bear_off(player):
            ...
            if len(higher_src) == 0:
                ...
                if s1_highest is not None:
                    ...
                    if dice_used == 0:
                            ...
                            if s2_highest is not None:
                                ...
                                if sum(self.board[s2_highest][0]) == 1:
                                    ...
                                    if s3_highest is not None:
                                        ...
                                        elif sum(self.board[s3_highest][0]) >= 2:
                                            plays.add((move1, move2, move3, move3))

                                elif sum(self.board[s2_highest][0]) == 2:
                                ...
                                elif sum(self.board[s2_highest][0]) >= 3:
                                    plays.add((move1, move2, move2, move2))

                        elif sum(self.board[s1_highest][0]) == 2:
                        ...
                        elif sum(self.board[s1_highest][0]) == 3:
                        ...
                        elif sum(self.board[s1_highest][0]) >= 4:
            ...
            if len(higher_src) == 1:
                if dice_used == 0:
                    ...
                    if self.is_valid(player, t1):
                        ...
                        if sum(self.board[high_src][0]) == 1:
                            ...
                            if comp3(player, s1, t1):
                                ...
                                if s1_highest is not None:
                                    if self.board[s1_highest][1] != player:
                                        c1 = sum(self.board[high_src][0])
                                    else:
                                        c1 = (sum(self.board[s1_highest][0]) + 1) if t1 == s1_highest else sum(self.board[s1_highest][0])
                                    ...
                                    if c1 == 1:
                                        ...
                                        if s2_highest is not None:
                                            if self.board[s2_highest][1] != player:
                                                ...
                                                c2 = (sum(self.board[s2_highest][0]) + 1) if t1 == s2_highest else sum(self.board[s2_highest][0])
                                            active_positions -= {s2_highest}
                                ...
                                if comp3(player, s1, t2):
                                    ...
                                    if s1_highest is not None:
                                        if self.board[s1_highest][1] != player:
                                            ...
                                            c1 = (sum(self.board[s1_highest][0]) + 1) if t2 == s1_highest else sum(self.board[s1_highest][0])
                                        active_positions -= {s1_highest}
                                        move_s1 = (s1_highest, clamp(s1_highest + r))
                        ...
                        elif sum(self.board[high_src][0]) == 2:
                        ...
                        elif sum(self.board[high_src][0]) == 3:
            ...
            if len(higher_src) == 2:
                if dice_used == 0:
                    ...
                    if (sum(self.board[high_src1][0]) + sum(self.board[high_src2][0])) <= 3:
                        ...
                        if self.is_valid(player, t11) and self.is_valid(player, t12):
                            ...
                            if comp3(player, s1, t11) and comp3(player, s1, t12):
                                ...
                                if s1_best is not None:
                                    if self.board[s1_best][1] != player:
                                        ...
                                        c1 = (sum(self.board[s1_best][0]) + 1) if t11 == s1_best else sum(self.board[s1_best][0])
                                        c1 = (c1 + 1) if t12 == s1_best else c1

                                    active_positions -= {s1_best}
                                    move_s1 = (s1_best, clamp(s1_best + r))

                                    if sum(self.board[high_src1][0]) == 1 and sum(self.board[high_src2][0]) == 1:
                                    ...
                                    elif sum(self.board[high_src1][0]) == 2 and sum(self.board[high_src2][0]) == 1:
                                        plays.add((move11, move11, move12, move_s1))

                                    elif sum(self.board[high_src1][0]) == 1 and sum(self.board[high_src2][0]) == 2:
                                        plays.add((move12, move12, move11, move_s1))

                            elif comp3(player, s1, t11) and sum(self.board[high_src2][0]) < 2:

                                if self.is_valid(player, t12) and comp3(player, s1, t22) and sum(self.board[high_src1][0]) < 2:
                            ...
                            elif comp3(player, s1, t12) and sum(self.board[high_src1][0]) < 2:

                                if self.is_valid(player, t11) and comp3(player, s1, t21) and sum(self.board[high_src2][0]) < 2:
                ...
                if dice_used == 1:
                    ...
                    if (sum(self.board[high_src1][0]) + sum(self.board[high_src2][0])) <= 2:
            ...
            if len(higher_src) == 3:
                if dice_used == 0:
                    ...
                    if (sum(self.board[high_src1][0]) + sum(self.board[high_src2][0]) + sum(self.board[high_src3][0])) <= 3:
    ...
    def can_move_to(self, player, target):
        ...
        return sum(self.board[target][0]) < 2 or (sum(self.board[target][0]) > 1 and self.board[target][1] == player)
    ...
    def render(self, round):
        ...
        print(f" \t\t\t  ROUND {round} ")
        print(" ========================================================== ")
        print("  WHITE | 0 | 1 | 2 |   | 3 | 4 | 5 |   | 6 | 7 | 8 | BLACK ")
        print(" ---------------------------------------------------------- ")
        print(f"    {sum(self.off[0])}   | {print_token(top_board[0][1])} | {print_token(top_board[1][1])} | {print_token(top_board[2][1])} |   | {print_token(middle_board[0][1])} | {print_token(middle_board[1][1])} | {print_token(middle_board[2][1])} |   | {print_token(bottom_board[0][1])} | {print_token(bottom_board[1][1])} | {print_token(bottom_board[2][1])} |   {sum(self.off[1])}")
